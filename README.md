...
_Please note that the above command has a **.** (dot) in the end, which is also to be included._
...
_Please note that RESTler (unlike other fuzzers) provides an in-built functionality to set the time. It will continue to execute till the set time expires._
